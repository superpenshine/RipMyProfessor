...
def index(request):
	...
	if request.method == 'GET':
		...
		if request.GET.get("action") == "search_by_userInput":
			clue = request.GET.get("userInput")
			#show all subjects if empty
			print("ready: "+clue)
			if clue == "": return render(request, 'rip/courses.html', {'courses': get_all_courses(),})
			return render(request, 'rip/courses.html', {'courses': get_courses_by_subject(clue),})
	#not POST nor GET
